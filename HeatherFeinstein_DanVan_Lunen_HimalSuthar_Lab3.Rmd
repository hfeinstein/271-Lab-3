...
# question requests fits using pre-2015 data
logseries1_train <- series1 %>% window(end=c(2014,4))
logseries1_test <- series1 %>% window(start=2015)

autoplot(decompose(diff(logseries1_train))) +
...
logseries1_diff1 <- diff(logseries1_train)

# add a seasonal difference
autoplot(decompose(diff(logseries1_diff1,4)))
```

Next lets attempt to remove the trend with differencing alone without the log tranformation. `ndiffs` indicated a first difference would be a good start according to the KPSS test.
...
Comparing our transformed (log10) and differenced series to our differenced alone series we prefer the latter as the residuals look more stationary and both resulted in a stable variance.
...
The auto arima residuals look like white noise.  The T plot looks approximately stationary and centered around 0.  This is confirmed by the histogram, which is centered around 0 and has an approximately normal distribution.  Finally, the ACF plot shows no significant correlations, indiciative of white noise.  

We'll run a L-Jung Box test to test the stastical significance of the ACF plot.  

```{r}

Box.test(auto.arima.model$residuals, lag = 24, type = c('Ljung-Box'))

```

Based on this test, we cannot reject $H_0$ that there is no correlation bewteen the lag values and the current values.  

We'll also run a Shapiro-Wilk test to test the normality of the residuals.  
...
lines(pred1, col = 'green', lty = 2 )
legend('topleft', legend = c('Original', 'Estimated', 'Forecasted'), col = c('black', 'red', 'green'), lty = c(1,2,2), cex = .8)

```

Based on thechart above, we see that our model's estimated values fit the training series (up to Q1, 2015) pretty well, with low deviation from the origianl values.  The forecasted values for the test set (2015 - 2016) also fit fairly well, following the overall trend and seasonality, though there seems to be more variance between the original values and the forecasted values. 

Let's now forecast through 2017 using our model.  

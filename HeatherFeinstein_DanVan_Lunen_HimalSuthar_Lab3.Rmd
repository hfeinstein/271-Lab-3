...
```{r}
df <- read.csv("ECOMPCTNSA.csv", header = FALSE, sep=",")
...
```{r}
df1 <- read.csv("AMAZ.csv", header = FALSE, sep=",")
df2 <- read.csv("UMCSENT.csv", header = FALSE, sep=",")

# Examine the data structure df1
str(df1)
names(df1)
head(df1)
tail(df1)

# Examine the data structure df2
str(df1)
names(df1)
head(df1)
tail(df1)

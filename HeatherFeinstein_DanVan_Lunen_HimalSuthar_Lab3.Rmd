...
p1 = autoplot(acf(series1_differenced, plot = FALSE)) +
  ggtitle("Correlation Functions\nDifferenced Series")

p2 = autoplot(pacf(series1_differenced, plot = FALSE)) +
  ggtitle("")
...
# train models
m_eda_max <- series1_train %>% Arima(order=c(1,1,1), seasonal = c(2,1,2), method='ML')
m_111111 <- series1_train %>% Arima(order=c(1,1,1), seasonal = c(1,1,1), method='ML')
m_110110 <- series1_train %>% Arima(order=c(1,1,0), seasonal = c(1,1,0), method='ML')
m_010010 <- series1_train %>% Arima(order=c(0,1,0), seasonal = c(0,1,0), method='ML')
m_auto_arima <- series1_train %>% auto.arima(stepwise=FALSE, approximation=FALSE)
summary(m_auto_arima)

m_forecast <- m_auto_arima %>% forecast(8)


# function to evaluate models
evaluate_model <- function(sarima_model){
  m_forecast <- sarima_model %>% forecast(8)
  rmse <- sqrt(mean((m_forecast$mean - series1_test)^2))
  return(c(sarima_model$aic,sarima_model$aicc,sarima_model$bic,rmse))
}

models <- list(m_eda,m_111111,m_110110,m_010010,m_auto_arima)

purrr::map(models,evaluate_model)
